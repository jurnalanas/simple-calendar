...
var datePicker = function datePicker(selector) {
  ...
  var applyFormatting = function applyFormatting(dateString, type, format) {
    if (type === types.TIME && dateString) {
      var _dateString$split$map = dateString.split(":").map(function (s) {
        return parseInt(s, 10);
      }),
          _dateString$split$map2 = _slicedToArray(_dateString$split$map, 2),
          h = _dateString$split$map2[0],
          m = _dateString$split$map2[1];

      return (0, _moment.default)().hour(h).minutes(m).format(format);
    }

    return (0, _moment.default)(dateString).format(format);
  };

  var prettifyDate = function prettifyDate(dateString, type) {
    var _DATE$TIME$DATETIME$t;

    var DATE = types.DATE,
        TIME = types.TIME,
        DATETIME = types.DATETIME;
    var format = (_DATE$TIME$DATETIME$t = {}, _defineProperty(_DATE$TIME$DATETIME$t, DATE, "ll"), _defineProperty(_DATE$TIME$DATETIME$t, TIME, "hh:mm A"), _defineProperty(_DATE$TIME$DATETIME$t, DATETIME, "ll hh:mm"), _DATE$TIME$DATETIME$t)[type];
    return applyFormatting(dateString, type, format);
  };

  var uglifyDate = function uglifyDate(dateString, type) {
    var _DATE$TIME$DATETIME$t2;

    var DATE = types.DATE,
        TIME = types.TIME,
        DATETIME = types.DATETIME;
    var format = (_DATE$TIME$DATETIME$t2 = {}, _defineProperty(_DATE$TIME$DATETIME$t2, DATE, "YYYY-MM-DD"), _defineProperty(_DATE$TIME$DATETIME$t2, TIME, "hh:mm"), _defineProperty(_DATE$TIME$DATETIME$t2, DATETIME, "YYYY-MM-DDThh:mm"), _DATE$TIME$DATETIME$t2)[type];
    return applyFormatting(dateString, type, format);
  };
...
var cal = {
  ...
  list: function list() {
    ...
    loadData(); // DRAWING CALCULATIONS
    // Determine the number of blank squares before start of month

    var squares = setUpCalendar(startDay, daysInMth, endDay); // DRAW Calendar

    drawCalendar(squares); // REMOVE ANY ADD/EDIT EVENT DOCKET
...
function loadData() {
  cal.data = localStorage.getItem("cal-".concat(cal.sMth, "-").concat(cal.sYear));

  if (cal.data == null) {
    localStorage.setItem("cal-".concat(cal.sMth, "-").concat(cal.sYear), "{}");
    cal.data = {};
  } else {
